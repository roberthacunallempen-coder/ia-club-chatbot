...
export default function FAQEdit() {
  ...
  const loadFAQ = async () => {
    try {
      const { data } = await client.get(`/api/faqs/${id}`);
      setForm(data);
    } catch (error) {
      toast.error('Error cargando FAQ');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await client.put(`/api/faqs/${id}`, form);
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Editar FAQ</h2>
